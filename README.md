# Hi, I'm Honglin Liao 👋

**English** · [中文](README.zh-CN.md)

**AI undergraduate · AI native since 2023 · Independent musician Lawrence River**

I build AI agent harnesses and explore computer vision, generative AI, reinforcement learning, and audio processing. Most things I build start with a small everyday annoyance and the thought: there has to be an easier way. I use AI for research, implementation, validation, and turning reliable workflows into reusable Skills.

### Latest builds

- [**Subtitle Overlay ↗**](https://github.com/LawrenceRiver/subtitle-overlay-mac) — Hide translated subtitles and practise listening without leaving the video.
- [**Spec Board ↗**](https://github.com/LawrenceRiver/spec-board) — Turn rough visual thinking into traceable, build-ready specifications.
- [**Distilled Admissions Agency ↗**](https://github.com/LawrenceRiver/distilled-admissions-agency) — Distill admissions research into an evidence-backed, locally runnable Codex Skill.

🌐 [Website](https://lawrenceriver.github.io/) · [Email](mailto:lawrencerivermusic@outlook.com) · [Spotify](https://open.spotify.com/artist/0qBwVBNlsXciLGqOvY1wwj) · [YouTube](https://www.youtube.com/@lawrenceriver_lhl) · [Bilibili](https://space.bilibili.com/3494359221667982) · [RedNote](https://www.xiaohongshu.com/user/profile/644cea2a00000000100282df)
